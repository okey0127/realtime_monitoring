...
#graph path
temp_path = now_dir+'/temp.jpg'
vib_path = now_dir+'/vib.jpg'

def temp_streamer():
    img = io.BytesIO()
    plt.plot(x_list, temp_data)
...
@app.route('/vib_graph')
def vib_graph():
    return Response(vib_streamer(), mimetype = "multipart/x-mixed-replace; boundary=frame")
